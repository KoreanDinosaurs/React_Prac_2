...
import { deleteDataFB, updateDataFB2 } from "./redux/modules/data";

function Chinese(){
    const dict_list = useSelector(state => state.data.list).filter(v => v.language === 'cn')
                ...
                return(
                    <Container num={completeNum} key={v.id}>
                        ...
                        <Wrap num={completeNum}>
                            <button onClick={click_btn} data-id={v.id}> 
                            ...
                            <button onClick={click_cancle} data-id={v.id}>
                            <svg stroke="currentColor" fill="currentColor" strokeWidth="0" version="1.2" baseProfile="tiny" viewBox="0 0 24 24" completed="true" height="1em" width="1em" xmlns="http://www.w3.org/2000/svg">
